...
describe("About Applying What We Have Learnt", function() {
  ...
  it("should add all the natural numbers below 1000 that are multiples of 3 or 5 (functional)", function () {
    ...
    // _(_.range(0,1000)).chain()
    //   .filter(function (x) {if (x % 3 === 0 || x % 5 === 0) {return x}})
      //.reduce(function (i, j) {return i += j})

    sum = _.range(0,1000).filter(function (x) {
      if (x % 3 === 0 || x % 5 === 0) {
        return x;
      }
    });

    sum =  sum.reduce(function (total, current) {
  ...
  it("should count the ingredient occurrence (functional)", function () {
    ...
    */
    ...
    _(products).chain()
      ...
      .flatten()
      .reduce(function(i, j) {return ingredientCount[j] = (ingredientCount[j] || 0) + 1})
      .value();
